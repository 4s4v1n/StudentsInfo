...
const UpdateModal = ({ active, setActive, value, mutateUpdate, selectedValue }) => {

    const [formData, setFormData] = useState({});

    const UpdateData = () => {
        let data = formData
        data = { ...data, [Object.keys(value[0])[0]]: selectedValue }
        mutateUpdate(data)
    }
                                            ...
                                            Object.keys(value[0]).map((key, index) => (
                                                <div>
                                                    ...
                                                    <TextInput
                                                        ...
                                                        placeholder={key}
                                                        value={index === 0 ? selectedValue : null}
                                                        onChange={(e) => setFormData({ ...formData, [key]: e.target.value })}>
                                                    </TextInput>
                                                </div>
                                            ))
                                        }
                                        <button onClick={() => { UpdateData(); setActive(false) }} type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Save all</button>
                                    </form>
                                </div>
                            </div>
                        </div >
